...
main () {
	...
	fputs( string, file);
 	...
 	i = BUS2BV_Dlya_provyerki; /* БУС2БВ_Для_проверки */
	fputs( "БУС2БВ_Для_проверки = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = BUS2BV_NYEISPRAVNOST__RUCHNOYE_UPRAVLYENIYE_VKLYUCHYENIYE; /* БУС2БВ_НЕИСПРАВНОСТЬ_РУЧНОЕ_УПРАВЛЕНИЕ_ВКЛЮЧЕНИЕ */
	fputs( "БУС2БВ_НЕИСПРАВНОСТЬ_РУЧНОЕ_УПРАВЛЕНИЕ_ВКЛЮЧЕНИЕ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = BUS2BV_NYEISPRAVNOST__RUCHNOYE_UPRAVLYENIYE_VYKLYUCHYENIYE; /* БУС2БВ_НЕИСПРАВНОСТЬ_РУЧНОЕ_УПРАВЛЕНИЕ_ВЫКЛЮЧЕНИЕ */
	fputs( "БУС2БВ_НЕИСПРАВНОСТЬ_РУЧНОЕ_УПРАВЛЕНИЕ_ВЫКЛЮЧЕНИЕ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = BUS2BV_NYEISPRAVNOST__AVTOMATICHYESKOYE_UPRAVLYENIYE_RUKI_POMYESCHYENY; /* БУС2БВ_НЕИСПРАВНОСТЬ_АВТОМАТИЧЕСКОЕ_УПРАВЛЕНИЕ_РУКИ_ПОМЕЩЕНЫ */
	fputs( "БУС2БВ_НЕИСПРАВНОСТЬ_АВТОМАТИЧЕСКОЕ_УПРАВЛЕНИЕ_РУКИ_ПОМЕЩЕНЫ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = BUS2BV_NYEISPRAVNOST__NYEISPRAVNOST__AVTOMATICHYESKOYE_UPRAVLYENIYE_RU; /* БУС2БВ_НЕИСПРАВНОСТЬ_НЕИСПРАВНОСТЬ_АВТОМАТИЧЕСКОЕ_УПРАВЛЕНИЕ_РУ */
	fputs( "БУС2БВ_НЕИСПРАВНОСТЬ_НЕИСПРАВНОСТЬ_АВТОМАТИЧЕСКОЕ_УПРАВЛЕНИЕ_РУ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);
