...
main () {
	...
	fputs( string, file);
 	...
 	i = AU2GUI_V_RUCHNOYE_UPRAVLYENIYE; /* АУ2ГУИ_В_РУЧНОЕ_УПРАВЛЕНИЕ */
	fputs( "АУ2ГУИ_В_РУЧНОЕ_УПРАВЛЕНИЕ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = AU2GUI_OTKLYUCHIT__RUCHNOYE_UPRAVLYENIYE; /* АУ2ГУИ_ОТКЛЮЧИТЬ_РУЧНОЕ_УПРАВЛЕНИЕ */
	fputs( "АУ2ГУИ_ОТКЛЮЧИТЬ_РУЧНОЕ_УПРАВЛЕНИЕ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = AU2GUI_VKLYUCHIT__SUSHILKU; /* АУ2ГУИ_ВКЛЮЧИТЬ_СУШИЛКУ */
	fputs( "АУ2ГУИ_ВКЛЮЧИТЬ_СУШИЛКУ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = AU2GUI_VYKLYUCHIT__SUSHILKU; /* АУ2ГУИ_ВЫКЛЮЧИТЬ_СУШИЛКУ */
	fputs( "АУ2ГУИ_ВЫКЛЮЧИТЬ_СУШИЛКУ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = KOM2AU_YEST__SUSHILKA_V_RUCHNOYE_UPRAVLYENIYE; /* КОМ2АУ_ЕСТЬ_СУШИЛКА_В_РУЧНОЕ_УПРАВЛЕНИЕ */
	fputs( "КОМ2АУ_ЕСТЬ_СУШИЛКА_В_РУЧНОЕ_УПРАВЛЕНИЕ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = KOM2AU_YEST__OTKLYUCHIT__RUCHNOYE_UPRAVLYENIYE; /* КОМ2АУ_ЕСТЬ_ОТКЛЮЧИТЬ_РУЧНОЕ_УПРАВЛЕНИЕ */
	fputs( "КОМ2АУ_ЕСТЬ_ОТКЛЮЧИТЬ_РУЧНОЕ_УПРАВЛЕНИЕ = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = KOM2AU_SUSHILKA_VYKLYUCHYENA; /* КОМ2АУ_СУШИЛКА_ВЫКЛЮЧЕНА */
	fputs( "КОМ2АУ_СУШИЛКА_ВЫКЛЮЧЕНА = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);

 	i = KOM2AU_SUSHILKA_VKLYUCHYENA; /* КОМ2АУ_СУШИЛКА_ВКЛЮЧЕНА */
	fputs( "КОМ2АУ_СУШИЛКА_ВКЛЮЧЕНА = " , file);
	sprintf(string, "%d\n", i );
	fputs( string, file);
