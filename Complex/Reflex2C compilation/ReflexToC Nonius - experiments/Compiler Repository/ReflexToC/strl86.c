...
void GenHeadersInProc (void)
{
	...
	sprintf(string, "#include \"%sext.inc\" /* Common files for all generated c-files */\n", BaseOutputFileName);
	fputs(string, fptr_C);
...
void GenVarDescription (void)
{
	...
	FILE *fptr_ports;     /* указатель на файл портов */
	...
	if ( Flag_TEST )
	{		
		strcpy(string, BaseOutputFileName);     /* создадим файл NAME_ports.ports */
		strcat(string, "ports.ports");
		Open_File_W(&fptr_ports, string);
	...
	if (First_Input_Group.name[0] != ZERO) {
		...
		for (; Current_Input_Group->logname != NOT_INITIALIZE; ) {
			...
			if ( Flag_TEST )
			{
				// ports & tests files generation
	
					char str[3];
					fputs(Current_Input_Group->name , fptr_ports);
					fputs("/*name*/, " , fptr_ports);
					sprintf(str, "%d", Current_Input_Group->logname);
					fputs( str, fptr_ports);
					fputs(" /*index*/, ", fptr_ports);
					sprintf(str, "%d", Current_Input_Group->size);
					fputs( str, fptr_ports);
					fputs(" /*size*/\n", fptr_ports);


					fputs("port: " , fptr_test);
					fputs( str, fptr_test);
