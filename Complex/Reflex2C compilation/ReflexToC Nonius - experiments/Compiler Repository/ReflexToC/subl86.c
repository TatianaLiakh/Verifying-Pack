...
// NEW 
		...
		printf("      =        -G      (Create output folder)                          =\n");
		printf("      =        -T      (Test-mode generation)                          =\n");
		printf("      =        -I      (Dont create _id.c file)                        =\n");
...
//END NEW
		...
		*/
		...
		for (i = 1; (i < (argc-1)) && (argv[i][0] == '-'); i++){   /* обработать блок опций командной строки */
			switch (argv[i][1]) 
			{    /* обработка опции */
			...
			case 'P': /* необходимо создать файлы в формате .cpp */
				switch (argv[i][2]){
				case ZERO:
					Flag_CPP = ON;      /* флаг cpp ON */
#ifdef TEST_MODE
					printf("\n%%RCSL-64-MSG, флаг теста ВКЛЮЧЕН");
#endif
					break;
				case '-':
					Flag_CPP = OFF;     /* флаг cpp OFF */
					break;
				default:
					printf("\n%%RCSL-64-FATERR, В командной строке обнаружен неизвестный ключ - %s", argv[i]);
					exit(!ZERO);
			...
			case 'G': /*  необходимо генерировать файл-описание портов */
				switch (argv[i][2]){
				case ZERO:
			...
			case 'I': /* Указываем, что файлы _id.c создавать не надо */
				switch (argv[i][2]){
				case ZERO:
					Flag_ID_FILE_NEED = OFF;    
#ifdef TEST_MODE
					printf("\n%%RCSL-64-MSG, флаг генерации описания портов ВКЛЮЧЕН");
#endif
					break;
				case '-':
					Flag_ID_FILE_NEED = ON;     
					break;
				default:
					printf("\n%%RCSL-64-FATERR, В командной строке обнаружен неизвестный ключ - %s", argv[i]);
					exit(!ZERO);
		...
		if (i != (argc-1) ) { /* пропущено имя файла */
		...
		if (strlen(BaseOutputFileName) > 255) {   /* если длина имени больше 4-х */
			printf("\n%%RCSL-64-FATERR, Длина имени выходного файла не должна превышать 255 символа - %s", BaseOutputFileName);
			exit(!ZERO);
		}
